...
FILENAME=$(date +"%Y-%m-%d_%H-%M-%S_bb.backup")
PGPASSWORD=$POSTGRES_PASSWORD pg_dump -U postgres -h localhost --file=/sik_data/$FILENAME -Fc -Z9 bb
...
if [ -f /sik_data/last.backup ]; then
  ...
  rclone copy /sik_data/$FILENAME dropbox:Apps/blue-borders
