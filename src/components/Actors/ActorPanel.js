...
const ActorPanel = ({ doc, index }) => {
  const store = useContext(storeContext)
  const { activeActor, getActor, toggleDraftOfActor } = store.actors
  const onClickActor = useCallback(
    e => {
      // prevent higher level panels from reacting
      e.stopPropagation()
      const idToGet = !activeActor || activeActor._id !== doc.id ? doc.id : null
      getActor(idToGet)
    },
    [activeActor, doc.id, getActor],
  )
